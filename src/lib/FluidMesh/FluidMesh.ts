...
import type { Cip113PolicyConfig, Cip113PolicyData } from './types';
import {
  FluidMeshError,
...
class FluidMesh {
  ...
  async isStakeCredentialRegistered(rewardAddress: string): Promise<boolean> {
    try {
      const accountInfo =
        await this.blockchainProvider.fetchAccountInfo(rewardAddress);
  ...
  async deployPolicy(
    ...
    FluidMeshResult<{ txHash: string; successCode: FluidMeshSuccessCode }>
  > {
    try {
      ...
      if (isRuleRegistered && isSmartTokenRegistered) {
        console.log(
          'Both stake credentials already registered, skipping deployment'
        );
        return createSuccessResult(
          {
            txHash: '',
            successCode: FluidMeshSuccessCode.CREDENTIALS_ALREADY_REGISTERED,
          },
  ...
  async mintPolicy(
    ...
    try {
      ...
      const allocations =
        recipientAllocations && recipientAllocations.length > 0
      ...
      if (allocations) {
        for (const allocation of allocations) {
          const recipientHash = deserializeAddress(
            allocation.address
          ).pubKeyHash;
      ...
      txBuilder
        ...
        .mintPlutusScriptV3()
        .mint(quantity, policyData.smartToken.policy, policyData.tokenNameHex)
   ...
   */
  async getCIP113TokensForAddress(smartReceiverAddress: string): Promise<
    FluidMeshResult<
      {
        unit: string;
        quantity: string;
  ...
  > {
    try {
      const utxos =
        await this.blockchainProvider.fetchAddressUTxOs(smartReceiverAddress);
  ...
  > {
    try {
      ...
      for (const policy of policies) {
        ...
        const result =
          await this.getCIP113TokensForAddress(smartReceiverAddress);
  ...
  /**
   * Transfer CIP113 tokens to multiple addresses
   * @param wallet - Browser wallet for signing
   * @param policyData - Policy data
  ...
  async transferCIP113Tokens(
    ...
    try {
      ...
      // Calculate change if needed
      const changeAmount = BigInt(currentBalance) - totalTransferAmount;
      if (changeAmount < 0) {
      ...
      // Add output to each recipient
      for (const recipient of recipients) {
        // Calculate recipient smart address
        const recipientHash = deserializeAddress(recipient.address).pubKeyHash;
        const recipientSmartAddress = this.getSmartReceiverAddress(
          policyData.smartToken.scriptCbor,
          recipientHash,
          false
      ...
      txBuilder
        ...
        .selectUtxosFrom(utxos)
        .txInCollateral(
          (await collateral).txHash,
          (await collateral).outputIndex
        )
        .requiredSignerHash(signerHash)
        .withdrawalPlutusScriptV3()
        .withdrawal(policyData.smartToken.rewardAddress, '0')
