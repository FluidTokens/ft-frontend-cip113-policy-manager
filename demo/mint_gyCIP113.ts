...
async function main() {
  ...
  let quantity_minted="1000"

  console.log(address_smart)

  const txBuilder = getTxBuilder();
  await txBuilder
    .txOut(address_smart,[{unit:smartToken.policy+tokenNameHex,quantity:quantity_minted}])
    ...
    .mintPlutusScriptV3()
    .mint(quantity_minted, smartToken.policy, tokenNameHex)
    .mintingScript(smartToken.scriptCbor)
